...
from .PLGeometricLib import Point, BBox
from .PLLocationConversion import PlaneLocation, GridLocation, BinLocation, ThreeDLocation, Bin2Grid, ThreeD2Grid
from .PLLocationConversion import Grid2Plane, Plane2Grid
    ...
    def twl(self, avg_sites_per_row, divide_factor_x, number_of_layers):
        ...
        for edgeObj in self.edges.values():
            ...
            if isinstance(p1, GridLocation):
                ...
                layer1, perlayer_bin1 = config.folded_bins_map.global_to_perlayer_bin(global_bin1)
                layer2, perlayer_bin2 = config.folded_bins_map.global_to_perlayer_bin(global_bin2)
...
class Node(object):
    ...
    def update_bbox(self):
        if isinstance(self.point_lb, PlaneLocation) or isinstance(self.point_lb, Point):
                ub = PlaneLocation(float(self.point_lb.x + self.width-1), float(self.point_lb.y + self.height/config.single_cell_height-1), 0.0)
                self.bbox = BBox(lb=self.point_lb, ub=ub, name=self.name)
        elif isinstance(self.point_lb, GridLocation):
                ub = GridLocation(int(self.point_lb.xgrid + self.width-1), int(self.point_lb.ygrid + self.height/config.single_cell_height-1), 0)
                self.bbox = BBox(lb=self.point_lb, ub=ub, name=self.name)
        elif isinstance(self.point_lb, BinLocation):
                #assume self.point_lb is Grid Location 
                #Add check
                ub = BinLocation(int(self.point_lb.bin_number), int(self.point_lb.yrow + self.height/config.single_cell_height), int(self.point_lb.xcolumn + self.width), 0)
                self.bbox = BBox(lb=self.point_lb, ub=ub, name=self.name)
        elif isinstance(self.point_lb, ThreeDLocation):
                #assume input is Bin Location
                #Add check
                ub = ThreeDLocation(int(self.point_lb.bin_number), int(self.point_lb.yrow + self.height/config.single_cell_height), int(self.point_lb.xcolumn + self.width), self.point_lb.zlayer)
                self.bbox = BBox(lb=self.point_lb, ub=ub, name=self.name)
        else:
