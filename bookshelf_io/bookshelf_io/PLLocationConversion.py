from .PLGeometricLib import Point
from .PLGridSpec import Grid, BinnedGrid, ThreeDBinnedGrid
...
class GridLocation(object):
    ...
    def check(self):
        if not isinstance(config.grid_definition, Grid):
        ...
        if self.xgrid < 0 or self.xgrid > config.grid_definition.avg_sites_per_row:
            print('number of columns=',config.grid_definition.avg_sites_per_row)
            raise ValueError("Grid location xgrid value is not valid. Found", self.xgrid, "Expected less than ", config.grid_definition.avg_sites_per_row)
        if self.ygrid < 0 or self.ygrid > config.grid_definition.numRows:
            print('numRows=',config.grid_definition.numRows)
            #raise ValueError("Grid location ygrid value is not valid. Found", self.ygrid, "Expected less than ", config.grid_definition.numRows)
        if not self.zgrid in config.layer_values:
            pass
...
class BinLocation(object):
    ...
    def check(self):
        if not isinstance(config.binned_grid_definition, BinnedGrid):
        ...
        if self.bin_number >= config.binned_grid_definition.numBins:
            raise ValueError("Bin location bin_number value is not valid Found", self.bin_number, "Expected less than or equal to ", config.binned_grid_definition.numBins)
        rowOBj = config.binned_grid_definition.rows[0]
        ...
        if not self.zlayer in config.layer_values:
            raise ValueError("Bin location zlayer value is not valid. Found", self.zlayer, "Expected one of ", config.layer_values)
...
class ThreeDLocation(object):
    ...
    def check(self):
        if not isinstance(config.threeD_binned_grid_definition, BinnedGrid):
            raise TypeError("Bin location grid_definition value is not BinnedGrid")
        if not isinstance(self.bin_number, int):
            raise TypeError("Bin location zbin value is not int")
        if not isinstance(self.yrow, int):
        ...
        if not isinstance(self.zlayer, int):
            raise TypeError("Bin location zlayer value is not int")
        if self.bin_number >= config.threeD_binned_grid_definition.numBins:
            raise ValueError("Bin location bin_number value is not valid Found", self.bin_number, "Expected less than or equal to ", config.threeD_binned_grid_definition.numBins)
        rowOBj = config.threeD_binned_grid_definition.rows[0]
        if self.yrow < 0 and self.yrow > rowOBj.height+1:
            raise ValueError("Bin location ycolumn value is not valid. Found", self.yrow, "Expected less than ", rowOBj.height+1)
        if self.xcolumn < 0 and self.xcolumn > rowOBj.width+1:
            raise ValueError("Bin location xrow value is not valid. Found", self.xcolumn, "Expected less than ", rowOBj.width+1)
...
class Grid2Plane(object):
    ...
    def update_plane_location(self):
        ...
        num_rows = config.grid_definition.numRows
        num_sites = config.grid_definition.rows[0].numSites
        ...
        if self.grid_location.ygrid == num_rows:
            row_object = config.grid_definition.rows[self.grid_location.ygrid-1]
            y = float(row_object.coordinate+row_object.height)
            x = float(row_object.subRowOrigin + row_object.width*self.grid_location.xgrid)
        else:
            row_object = config.grid_definition.rows[self.grid_location.ygrid]
...
class Plane2Grid(object):
    ...
    def update_grid_location(self):
        ...
        row_object = config.grid_definition.rows[0]
        ygrid = int((self.plane_location.y - row_object.coordinate) / config.single_cell_height)
...
class Bin2Grid(object):
    ...
    def update_grid_location(self):
        ...
        if not isinstance(config.grid_definition, Grid):
            raise TypeError("PLconfig_grid.grid_definition is not of type Grid")
        bbox = config.binned_grid_definition.squares.squares[self.bin_location.bin_number]
...
class Grid2Bin(object):
    ...
    def update_bin_location(self):
        ...
        if not isinstance(config.grid_definition, Grid):
            raise TypeError("PLconfig_grid.grid_definition is not of type Grid")
        b1 = config.binned_grid_definition.squares.findBin(Point(self.grid_location.xgrid, self.grid_location.ygrid))
        print(config.binned_grid_definition.squares)
        print(config.binned_grid_definition)
        c1=self.grid_location.xgrid%config.binned_grid_definition.divide_factor_x
        r1=self.grid_location.ygrid%config.binned_grid_definition.divide_factor_y
        #AM TODO: z transformation to be defined
        zgrid=0
...
class Bin2ThreeD(object):
    ...
    def update_threeD_location(self):
        ...
        if not isinstance(config.threeD_binned_grid_definition, ThreeDBinnedGrid):
            raise TypeError("PLconfig_grid.trheeDbinned_grid_definition is not of type ThreeDBinnedGrid")
        if not isinstance(config.binned_grid_definition, BinnedGrid):
            raise TypeError("PLconfig_grid.grid_definition is not of type Grid")
        threeD_bin_number = int(config.folded_bins_map.twoD_to_threeD(self.bin_location.bin_number))
...
class ThreeD2Bin(object):
    ...
    def update_twoD_location(self):
        ...
        twoD_bin_number = config.folded_bins_map.threeD_to_twoD(self.threeD_location.bin_number)
...
class ThreeD2Grid(object):
    ...
    def update_grid_location(self):
        ...
        if not isinstance(config.binned_grid_definition, BinnedGrid):
            raise TypeError("PLconfig_grid.grid_definition is not of type BinnedGrid")
        bbox = config.threeD_binned_grid_definition.squares.squares[self.threeD_location.bin_number]
...
class ThreeD2Plane(object):
    ...
    def update_plane_location(self):
        ...
        #bbox = config.threeD_binned_grid_definition.squares.squares[perlayer_bin_number]
        bbox = config.threeD_binned_grid_definition.squares.squares[self.threeD_location.bin_number]
        #print(bbox)
        xgrid = bbox.lb.x + self.threeD_location.xcolumn
        ...
        #origin = config.grid_definition.rows[0]
        if ygrid == num_rows:
            row_object = config.grid_definition.rows[ygrid-1]
            y = float(row_object.coordinate+row_object.height)
            x = float(row_object.subRowOrigin + row_object.width*xgrid)
        else:
            row_object = config.grid_definition.rows[ygrid]
            y = float(row_object.coordinate)
            x = float(row_object.subRowOrigin + row_object.width*xgrid)
...
class Plane2ThreeD(object):
    ...
    def update_threed_bin_location(self):
        ...
        row_object = config.threeD_binned_grid_definition.rows[0]
        number_of_layers = config.threeD_binned_grid_definition.number_of_layers
        divide_factor = config.divide_factor
        ...
        b1 = config.threeD_binned_grid_definition.squares.findBin(Point(xgrid, ygrid))
        c1=xgrid%config.threeD_binned_grid_definition.divide_factor
        r1=ygrid%config.threeD_binned_grid_definition.divide_factor
